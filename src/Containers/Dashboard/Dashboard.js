...
class Dashboard extends React.Component {
  constructor(props) {
    ...
    setTimeout(() => {
      userService.fetchBalanceScoreMonthData({ "date": new Date() }).then((data) => {
        this.setState({ allBalances: data })
        this.setState({ markedArray: this.markedDates() })
  ...
  markedDates() {
    let markedArray = [];
    let color="";
    console.log("all balance", this.state.allBalances);
    
    this.state.allBalances.map((item, index) => {
          ...
          <div className="row">
            ...
            <div className='col-md-12 cal'>
              <CalendarStrip
                ...
                markedDates={this.state.markedArray}
...
Dashboard.propTypes = {
  ...
  fetchBalanceScoresMonths: PropTypes.func,
...
const mapStateToProps = (state) => ({
  ...
  balanceScoresMonth: state.example.balanceScoresMonth,
